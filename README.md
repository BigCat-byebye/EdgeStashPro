...
- 后台任务：上传、下载、复制和移动会进入顶部任务状态栏，可查看进度、历史状态、停止或删除任务。
...
上传、下载、复制和移动都会创建文件任务，任务记录保存在 D1 中。页面顶部会在“刷新 / 管理后台”旁显示任务状态入口，点击后可以查看最近任务、状态、失败原因，并对任务执行停止或删除。
